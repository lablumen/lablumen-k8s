...
Value precedence (low → high): `charts/microservice/values.yaml` → `services/<svc>/values.yaml` →
...
# Lint (pass a service's values — the chart requires .Values.name to be set)
